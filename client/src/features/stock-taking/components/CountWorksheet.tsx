...
export function CountWorksheet() {
  ...
  const getDiscrepancy = (item: { id: string; systemQuantity: number }) => {
  ...
  if (isLoading)
    return <div className="p-6 text-center text-gray-400">Loading inventory data...</div>;
    ...
    <div className="w-full overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
      <div className="border-b border-gray-200 p-6">
        ...
        <p className="text-sm text-gray-500">
          Enter actual counts and provide a reason for any discrepancies.
        </p>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {items.map((item) => {
              ...
              const isReconciled =
                item.actualQuantity !== null || editableCounts[item.id] !== undefined;

              return (
                <tr
                  key={item.id}
                  className={isReconciled ? 'bg-gray-50/50' : 'hover:bg-gray-50/30'}
                >
                  ...
                  <td className="px-6 py-4 text-sm">
                    <input
                      ...
                      className={`w-20 rounded-md border px-2 py-1 text-sm shadow-sm focus:ring-1 focus:outline-none ${
                        editableCounts[item.id] === undefined
                          ? 'border-gray-300'
                          : 'border-blue-300'
                      }`}
                      placeholder="Count"
      ...
      {/* Submission Footer */}
      {selectedItemId && (
        <div className="flex flex-wrap items-end justify-between gap-4 border-t border-gray-200 bg-gray-50/80 p-6">
          <div className="min-w-[200px] flex-1">
            <label className="mb-1 block text-sm font-medium text-gray-700">
              Reason for adjustment (required if discrepancy exists)
            </label>
            <textarea
          ...
          <button
            ...
            disabled={isPending}
            className="inline-flex items-center gap-2 rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none disabled:opacity-50"
          >
