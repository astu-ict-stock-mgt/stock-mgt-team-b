...
// Central error helper to translate known database errors into HTTP responses.
const handleControllerError = (
  ...
  if (isPrismaLikeError(error)) {
    ...
    // P2023 / PostgreSQL UUID conversion errors.
    if (
      error.code === 'P2023' ||
      ...
      res.status(400).json({
        status: 'error',
        message:
          'Bad Request: The provided identifier does not match the required format.',
      });
      return;
    }
  }

  // Unknown errors are handled by the global error middleware.
  next(error);
};
...
export const getInventoryItemStockLotById = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const getInventoryItemStockLotValuation = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const createStockLotController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  ...
  try {
    ...
    if (!req.body || Object.keys(req.body).length === 0) {
      res.status(400).json({
        status: 'error',
        message:
          'Bad Request: No update data fields were provided in the request body.',
      });
      return;
    }
...
export const updateStockLotController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    ...
    if (!req.body || Object.keys(req.body).length === 0) {
      res.status(400).json({
        status: 'error',
        message:
          'Bad Request: No lot update properties were specified in the request body.',
      });
      return;
    }
...
export const deleteStockLotController = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
